...
public class Agent implements Cloneable{
	 ...
	 *  ロールのリスト
	 */
	private ArrayList<Role> havingRoles;
	/**
	 *  死んだロールリスト
	 */
	 ...
	 */
	public Agent(int life,int growRoleNum){
		this.life = life;
		this.age = 0;
		this.havingRoles = new ArrayList<Role>();
		this.deadRoles = new ArrayList<Role>();
	...
	public Object clone(){
		try{
			...
			// 参照型のフィールドにもclone
			clone.deadRoles = new ArrayList<Role>();
			for(Role role:this.deadRoles){
				clone.deadRoles.add((Role)role.clone());
			}
			clone.havingRoles = new ArrayList<Role>();
			for(Role role:this.havingRoles){
				clone.havingRoles.add((Role)role.clone());
			}
			return clone;
